/* ===========================================================
   SBS Painel de P&D / Inovação — núcleo
   Pipeline de projetos, ensaios de campo, cultivares, banco de
   ideias, cronograma e documentos. Compartilha a nuvem/store SBS.
...
const PD = (function(){
  ...
  function seed(){
    if(!S || !D) return;
    if(!(S.getCol("pd_projetos")||[]).length)   S.setCol("pd_projetos",   D.projetos.map(c=>Object.assign({},c)));
    if(!(S.getCol("pd_ensaios")||[]).length)    S.setCol("pd_ensaios",    D.ensaios.map(c=>Object.assign({},c)));
    if(!(S.getCol("pd_cultivares")||[]).length) S.setCol("pd_cultivares", D.cultivares.map(c=>Object.assign({},c)));
    if(!(S.getCol("pd_ideias")||[]).length)     S.setCol("pd_ideias",     D.ideias.map(c=>Object.assign({},c)));
    if(!(S.getCol("pd_marcos")||[]).length)     S.setCol("pd_marcos",     D.marcos.map(c=>Object.assign({},c)));
    if(!(S.getCol("pd_docs")||[]).length)       S.setCol("pd_docs",       D.docs.map(c=>Object.assign({},c)));
  }
  ...
  const NAV = [
    { sec:"Pesquisa & Desenvolvimento" },
    { id:"visao",      label:"Visão Geral",         icon:"gauge" },
    { id:"pipeline",   label:"Pipeline de Projetos", icon:"git-branch" },
    { id:"ensaios",    label:"Ensaios de Campo",    icon:"flask-conical" },
    { id:"cultivares", label:"Cultivares",          icon:"sprout" },
    { id:"ideias",     label:"Banco de Ideias",     icon:"lightbulb" },
    { id:"cronograma", label:"Cronograma & Marcos", icon:"calendar-clock" },
    { id:"documentos", label:"Documentos & Laudos", icon:"file-text" },
    { sec:"Apoio" },
  ...
  function go(id){
    ...
    document.getElementById("pg-crumb").textContent="P&D · "+m.label;
  ...
  function startSession(email){
    ...
    document.getElementById("sb-role").textContent="P&D / Inovação";
    ...
    go("visao"); window.SBS_AVATAR&&SBS_AVATAR.setUser(email); try{ localStorage.setItem("sbs_pd_user",email); }catch(e){}
  }
  function logout(){ try{ localStorage.removeItem("sbs_pd_user"); }catch(e){} session=null; var p=document.getElementById("lg-pass"); if(p)p.value=""; showLogin(); }
  function initLogin(){
